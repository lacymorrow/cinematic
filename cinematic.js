...
var settings = {
    /*
      set to false to remove demo purposes! [default: 'public/demo/']
    */
    DEMO: 'public/demo/',
    //
...
if (Meteor.isClient) {
  ...
  // });
    ...
    Template.path.events = {
        ...
        "click #fileDialog": function(event) {
            // Choosse files to load

            // default action is to open, this is redundant
            // choosePath('#fileDialog');
        },

        "change #fileDialog": function(event) {
    ...
    Meteor.methods({
        ...
        findMovieDir: function() {
            /* 
              ...
              Hack
              Attempts to figure out absolute directory of the running path (demo), 
              if not falls back to assumed movies directory. NEEDS TESTING.

            */

            var abs = process.cwd().replace(/[\d]*.meteor\/local\/build\/programs\/server\/?/, '');
            var home = process.env[(process.platform == 'win32') ? 'USERPROFILE' : 'HOME'];
            if (home.slice(-1) != '/') {
                home = home + '/';
            }

            // dir is the directory containimg movie files; use DEMO if you'd  trefer to test with dummy data
            var dir;
            if (abs.toLowerCase().indexOf('cinematic') >= 0 && settings.DEMO) {
                dir = abs + settings.DEMO;
            } else if (!settings.DEMO) {
                dir = abs; // current running location
            } else {
                dir = home;
            }
            ...
            files.forEach(function(file, i) {
                ...
                if (stats.isDirectory() && (file.toLowerCase().indexOf('movies') != -1 || file.toLowerCase().indexOf('videos') != -1)) {
                    dir = (settings.DEMO) ? settings.DEMO : dir + file + '/'; //DEMO
                }
            });
            broadcast('Cinematic: Using ' + dir + ' as initial movie directory');
            return dir;
        },
