...
const mainNotification = (options: NotificationOptions) => {
	Logger.info(`Sending notification to main process: ${options.title} - ${options.body}`);
	new Notification(options).show();
}

const rendererNotification = (options: NotificationOptions) => {
	Logger.info(`Sending notification to renderer process: ${options.title} - ${options.body}`);
