...
import {
	RATINGS_ROTATOR_INTERVAL,
	ROTATOR_TRANSITION_DURATION,
} from '@/renderer/config/config';
...
const RatingsRotator: React.FC<{ ratings: Rating[] }> = ({ ratings }) => {
	...
	if (!currentRating) {
		console.warn(
			`RatingsRotator: currentRating at index ${currentIndex} is null or undefined.`,
		);
		return null;
	}
	...
	const starCount =
		Math.round((currentRating.score / sanitizedMaxScore) * 5 * 2) / 2; // Half-star precision
	...
	return (
		<div className="flex flex-col justify-center items-center min-w-40">
			<AnimatePresence mode="wait">
				...
				>
					<div className="flex gap-2">{getStarIcons(starCount)}</div>
